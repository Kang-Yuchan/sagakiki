...
export async function GET(req: NextRequest) {
  ...
  const res = await axios.get(`${process.env.RAPIDAPI_URL}?term=${term}`, {
    headers: {
  ...
  if ('tracks' in res.data) {
    const hits: SongInfo[] = (res.data.tracks.hits as any[])
      .map(({ track }) => {
        return {
      ...
      .filter((item, index, array) => {
        return (
          index === array.findIndex(({ artist }) => artist === item.artist)
        );
      });
